    ...
    def vis_sim_one_day(self, output = False, output_dict = None):
        ...
        if options["display"]:
            if options["temporals"]:
                fig,(ax1,ax2,ax3,ax4,ax5) = plt.subplots(5,1,figsize=(19.2,10.8))
            ...
            if options["choices"]:
                ...
                plt.annotate("charging_flex",(1.5, 0.04))
                plt.annotate("charging asap",(1.5, sum(choice_probs[0][:2]) - 0.04))
                plt.annotate("leaving without charging", (1.5, sum(choice_probs[0][:3]) - 0.04))
