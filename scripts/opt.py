...
class Optimization:
    ...
    def run_opt(self):
        ...
        while (count < itermax) & (improve >= 0) & (abs(improve) >= self.Parameters.opt_eps):
            ...
            print(count)
        self.opt_z = zk
        self.opt_tariff_flex = zk[0]
        self.opt_tariff_asap = zk[1]
        self.opt_tariff_overstay = zk[2]
        self.opt_x = xk
        # update demand charge
        self.opt_peak_pow = max(xk[self.Problem.N_flex + 1:])
        self.opt_flex_SOCs = xk[0:self.Problem.N_flex + 1]
        self.opt_flex_powers = xk[self.Problem.N_flex + 1:]
        self.opt_asap_powers = np.ones((self.Problem.N_asap, 1)) * self.Problem.station_pow_max
        self.opt_v = vk
        self.opt_prob_flex = vk[0]
        self.opt_prob_asap = vk[1]
        self.opt_prob_leave = vk[2]
        self.opt_J = Jk[0:count-1]
        ...
        if self.Parameters.VIS_DETAIL:
            print(
                ' {0} OPT DONE ({1} sec) sum(vk) = {2}, iterations = {3}\n'.format(datetime.datetime.now(), end - start,
                                                                                   sum(vk), count))
            print("zk is {0}".format(self.opt_z))
            print("xk is {0}".format(self.opt_x))
            print("vk is {0}".format(self.opt_v))
